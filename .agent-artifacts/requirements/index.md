...
* [Output](output/) - Canonical generated BA deliverables, including discovery sessions, epics, user stories, GUI specs, and diagrams.

Recommended workflow:

1. Read `input/` for raw client requirement material.
2. Create elicitation session files in `output/elicitation/` (or directly within the target epic); store candidate PRDs in `drafts/candidate-specs/`.
3. Read generated hierarchy files under `output/elicitation/` or `output/<epic-slug>/` when they relate to the input.
