...
- **Avoid loose inline HTML tags (e.g. `<b>`, `<i>`) in `value` attributes:** draw.io decodes `&lt;b&gt;` as literal text `<b>`, displaying raw HTML tags on shapes. Use plain text formatting, section dividers (`---`), or cell-level styling (`fontStyle=1`).
- **Avoid wordiness and arbitrary step numbers in activity boxes:** Keep action labels concise, direct, and uncluttered (e.g., "Create Session" instead of "1. Create Session"). Rely on directional connectors to show sequence; omit step numbers unless explicitly requested by the user.
...
- **Avoid "Up-then-down" loops on swimlane return paths:** when connecting a node in an upper swimlane back to a lower swimlane, exit from the **BOTTOM** of the upper node (`exitX=0.5;exitY=1`) and enter the **LEFT** side of the lower node (`entryX=0;entryY=0.5`). Never exit top/right with hardcoded waypoints that force an arch loop into the top margin.
- **Stagger columns around decision gateways:** when a vertical handoff line passes a decision gateway, place the handoff nodes in a dedicated column offset by $\ge 120\text{px}$ to the right/left of the gateway. Align source and target nodes to the exact same X-center for a 100% straight vertical line that never crosses or loops over the decision diamond.
- Add `<Array as="points">` waypoints ONLY when an edge must detour around an unavoidable intermediate shape. Avoid over-constraining edges with unnecessary waypoints.
...
- **Prevent Decision Gateway Text Overlaps**: maintain a horizontal gap of $\ge 120\text{px}$ between a decision diamond and downstream process boxes. Always set `labelBackgroundColor=#ffffff;fontSize=11` on decision output edges.
