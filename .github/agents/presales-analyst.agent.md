...
- A genuine `requirements-elicitor` checkpoint means asking the actual user live, in the current conversation (for example, via a direct interactive question tool). If `requirements-elicitor` is invoked as a stateless subagent, treat its output only as draft candidate questions still to be posed to the user — never as answered or confirmed. Verify any "client answer" content it returns traces to the actual user or existing source material before relying on it; otherwise, ask the user directly instead.
...
Use `.github/skills/write-wbs/SKILL.md`. Do not invent effort values. If `.github/skills/write-wbs/SKILL.md` cannot be read, notify the user that the WBS skill file is missing and ask them to provide it or confirm they want to proceed with a default table structure (Feature | Description | Effort | Notes), leaving effort blank.
