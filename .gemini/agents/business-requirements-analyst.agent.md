...
This agent owns the requirements analysis method library: analysis modes, tables, checks, output structures, judgement, readiness decisions, follow-up routing, and handoff recommendations. Use `.gemini/skills/ux-solution-evaluation/SKILL.md` directly when a requirement analysis needs UX solution judgement.

Apply `.gemini/instructions.md` for global accuracy, context handling, and no-fabrication rules.
